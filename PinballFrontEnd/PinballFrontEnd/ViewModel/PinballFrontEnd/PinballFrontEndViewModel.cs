...
namespace PinballFrontEnd.ViewModel
{
    ...
    public partial class PinballFrontEndViewModel : ViewModelBase
    {
        ...
        private void InitalizeWindows()
        {
            ...
            //Backglass Window
            BackglassWindow = new View.MediaView(this);
            ...
            //DMD Window
            DMDWindow = new View.MediaView(this);
        ...
        private async Task ExitTableManager()
        {
        ...
        private async void ExitTable()
        {
            ...
            try
            {
                ...
                logger.Info($"Foreground Title: {WindowControl.GetForegroundWindowText()}");
        ...
        //Load a BitmapImage into memory
        private BitmapImage LoadBitMapImage(int scale, Uri source)
        {
            if (!File.Exists(source.LocalPath))
            {
                logger.Info($"Thumbnail Does Not Exist: {source.LocalPath}");
                var EmptyImage = new BitmapImage();
                EmptyImage.Freeze();
                return EmptyImage;
                //source = new Uri($@"{ProgramPath.Value}\Media\default.png");
            }
            logger.Info($"Loading Thumbnail: {source.LocalPath}");
            BitmapImage bi = new BitmapImage();
            bi.BeginInit();
            bi.DecodePixelWidth = scale;
        ...
        private async Task<BitmapImage[]> InitalizeThumbnails()
        {
            ...
            {
                //tasks.Add(Task.Run(() => pt.LoadThumbnails(Data.MediaLocation.ThumbnailResolution)));
                tasks.Add(Task.Run(() => LoadBitMapImage(Data.MediaLocation.ThumbnailResolution, pt.PlayfieldImage)));
                tasks.Add(Task.Run(() => LoadBitMapImage(Data.MediaLocation.ThumbnailResolution, pt.BackglassImage)));
                tasks.Add(Task.Run(() => LoadBitMapImage(Data.MediaLocation.ThumbnailResolution, pt.DMDImage)));
                tasks.Add(Task.Run(() => LoadBitMapImage(Data.MediaLocation.ThumbnailResolution, pt.Wheel)));
            }
        ...
        //Loads thumbnails of all tables into memory
        private async Task InitalizeThumbnails1()
        {
            logger.Info($"{RunMode.ToString()} : Loading Thumbnails");

            var Thumbnails = await InitalizeThumbnails();
            int cnt = 0;

            foreach (PinballTable pt in Data.TableList)
            {
                pt.PlayfieldThumbnail = Thumbnails[cnt];
                pt.BackglassThumbnail = Thumbnails[cnt + 1];
                pt.DMDThumbnail = Thumbnails[cnt + 2];
                pt.WheelThumbnail = Thumbnails[cnt + 3];
        ...
        public delegate void TableChangedEventHandler(object sender, EventArgs e);
        ...
        private void NextTable()
        {
            ...
            ChangeBGMusic();
            OnTableChanged(); //raise event
            //TableChanged(this, new PinballFrontEndTableChangedEventArgs()); //Raise Event
        }
